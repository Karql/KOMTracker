...
- **Component** — a wearable/replaceable part (chain, tyre, wheel, bar tape…) with a category. *(Standalone non-part cost items — tools, lube, clothing — are **Expenses** (D-20), a separate entity, not components.)*
- **Expense** — a standalone cost entry (lube, apparel, tools…) with an amount + date, not tied to a bike or component (D-20).
...
Per component/bike and per athlete: purchase + services − resale = **net cost**; cost/km; cost/period. Bike-level rollups may include attached components. Also unions standalone **expenses** (`bt.expense`, D-20). *(OQ-16: rollup rules.)*
...
- **D-4 Accessories/tools/consumables = component categories** (never installed) — avoids a parallel entity; still cost-tracked. **(Revised by D-20:** standalone non-part expenses now live in a dedicated `bt.expense` entity/page; D-4 stands only for items that are genuine installable/tracked parts you also cost-track.**)**
...
- **D-20 Standalone expenses = dedicated `bt.expense` entity + own page (revises D-4).** Pure cost entries with no part semantics — chain lube, apparel, tools, socks — are stored in a new minimal, User-owned `bt.expense` table (fields: **Name, Amount, Date, optional Category** [small enum: Consumable / Apparel / Tools / Accessories / Other], **optional Place, optional Notes**) and shown on their **own Expenses page**, never in the components list. No lifecycle, no bike link, no mileage/installation. *Why: the user has 200+ such rows; folding them into `ComponentEntity` (D-4) swamps the components list — whose fixed Location / Distance / Meta columns are meaningless for a purchase — and forces special-casing Component across query/list/detail/delete for zero gain. A separate minimal entity (cloned from the Warehouse CRUD vertical) is cleaner. Feeds Phase 4 cost analysis (§8) alongside component costs.* **Bulk-import of the Excel backlog is a separate later concern** (no import mechanism exists in the repo yet). Post-v1 phase; not built yet.
