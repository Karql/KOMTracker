...
- Spec-driven workflow: feature specs/checklists live in **`.ai/plans/`** (dated `YYYY-MM-DD-<slug>.md`, checklist format with a `Decisions` section). Tick items off during implementation.
  - **On plan approval, the first implementation step is to persist the spec to `.ai/plans/YYYY-MM-DD-<slug>.md`** (so every feature is documented in the repo, even if implementation is deferred).
