...
### Features
- BikeTracker (Strava webhooks — inbox): a public callback endpoint (`strava/callback/{secret}`) now receives Strava's push events — the subscription-validation handshake plus create/update/delete notifications — and stores each one raw in a new `strava.webhook_event` table, returning 200 immediately. This first step only **collects** events (to see what actually arrives); acting on them (near-real-time sync) comes later. The endpoint is guarded by a private secret in the URL (Strava doesn't sign its requests) and the standard `verify_token`; both live in config. The push subscription itself is registered manually (Postman).
- BikeTracker (currency + input polish): you can now pick a **preferred currency** on **Account → Profile details** (CHF / EUR / GBP / USD / PLN, default PLN); bike and component prices then show with that currency's symbol (e.g. `1500 zł`, `$1500`). Price and other decimal fields now accept **both `,` and `.`** as the decimal separator (typing "12,50" no longer drops the decimals). Date pickers use a consistent **`yyyy-MM-dd`** format and are **editable** — you can type or paste a date instead of clicking through the calendar (handy for old dates).
- BikeTracker (autocomplete for Brand / Model / Purchase place): the add/edit dialogs for bikes and components now suggest values you've used before — pooled from all your bikes and components — as you type, so repeat shops and manufacturers are a click instead of retyping (and stay spelled consistently). Any brand-new value is still accepted. Fed by a new `GET bike-tracker/suggestions` loaded when the dialog opens.
- BikeTracker (faster add & install): adding a component now flows straight into installing it — after you save, the install dialog opens with the component already picked (Cancel just leaves it created). The install dialog gained an optional **Uninstall date**, so a historical, already-removed part goes in as one closed entry instead of add → install → open detail → remove. From a bike's detail page there's a new **Add component ▾** button: *Add component* (creates, then installs on that bike) and *Add OEM component* (for parts that came with the bike — pre-fills the bike's purchase place/date, price 0, and defaults the install date to the bike's purchase date). The old **Remove** action (which just closes an installation window) is renamed **Uninstall** everywhere, to distinguish it from deleting the record or the component.
- BikeTracker (component delete — clearer block + force option): deleting a component that has installation history stays guarded (the safe default steers you to **Archive**), but the component page now shows **why** it was blocked instead of a bare "409", and — for a **standalone** component — offers a **Delete anyway** confirmation that also drops its own installation records, so you can clear a test component without unpicking each installation by hand. A **meta component that holds or ever held parts** is deliberately *not* force-deletable: dropping it would erase those parts' own history (e.g. a tyre briefly mounted in a wheel, later run on its own), so you're told to remove/delete the parts first (deleting a part clears its "in this component" record) or archive the container instead.
- BikeTracker (Phase 3 — component mileage): every component now shows how far it's actually ridden — **distance / moving-time / elevation** computed from the Strava rides that reached it **through its installation chain** (a chain on a bike gets that bike's rides during its install window; a tyre in a wheel gets the wheel-bike rides during the tyre-in-wheel overlap; a part on several bikes sums them). Shown as a **Mileage** panel and a distance column/chip on the components list, plus a **per-window** figure on each installation row and in the Contains panel (how much that placement delivered). Totals = **initial seed + manual baselines + attributed rides**, counting all rides like bike mileage does. Because grouping thousands of activities per component on every list load would be far too slow, totals are a **stored projection** (`bt.component_mileage`) recomputed from source — always fresh: it rebuilds inline whenever a component/installation/lifecycle changes (the component + its nested children) and after every activity sync (the components on the synced athlete's bikes, so a deleted old ride correctly drops its km). Editing an install date re-derives the totals with no drift. There's also an admin endpoint (`PUT admin/recalculate-component-mileage`, component ids in the body) to backfill/rebuild on demand.
- BikeTracker (Phase 2b-ii — meta-components & multi-bike): a component can now be installed **into another component**, not just onto a bike — model the real chain **tyre → wheel → bike**. A component's detail page shows its immediate **parent** (bike or component, linked) and a **Contains** panel listing what's mounted inside it; the install / move / edit dialogs gained an *On a bike / In a component* target toggle. Parts that rotate across bikes (a computer, pedals) can be installed on **several bikes at once**. The full linkage rules (**D-7**) are enforced: a component is either inside **one** parent component **or** on **one-or-more distinct** bikes — never mixed, never the same bike twice, no self/cyclic install, and nesting is capped at **one level** (no wheel-in-wheel-in-tyre towers) — each blocked case gives a clear message. Lifecycle cascades (**D-18**): selling a parent (e.g. a wheelset) marks the components inside it **sold** too and closes their windows; archiving a parent **detaches** them (they become unassigned, history kept); a component can't be hard-deleted while it has installation history as a part **or** as a parent (archive instead). The components list chip/filters understand the new states (*In {component}* / *On N bikes*). Components can be flagged **"meta component"** (a container, e.g. a wheel) — only these are offered as install targets, so the picker isn't cluttered by every single part; the list has a **Meta / Regular** filter and a Meta chip, and you can't switch a component out of meta while something's installed in it. The component **Contains** panel now shows **current + historical** children (like the installations table — current highlighted, past below). Computing how far each component has travelled through the chain is the next phase.
