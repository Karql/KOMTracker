...
- **`strava.bike`** (1c-i) — the athlete's Strava gear synced **1:1** (key = gear id, FK to `athlete`; name/nickname/primary/retired/distance + hydrated brand/model/frame_type/weight from `GET /gear/{id}`), incl. retired. `IGearService.GetAthleteBikesAsync(athleteId, token, extraGearIds)` hydrates the **union** of `GET /athlete` `bikes[]` (active only) and `extraGearIds` — the distinct bike (`b…`) `gear_id`s from `strava.activity` (`EFActivityRepository.GetDistinctBikeGearIdsAsync`) — so **retired/historical bikes** (omitted from `bikes[]`) are imported once their rides have synced. `EFStravaBikeRepository` bulk-upserts + deletes gear Strava no longer returns.
...
- **Jobs**: `SyncActivitiesRecentJob` (`After=now-7d`, Mon–Sat 01:35) + `SyncActivitiesFullJob` (`After=null`, Sun 01:35), gated by `SyncActivitiesJobEnabled`; `SyncBikesJob` (02:35, `SyncBikesJobEnabled`) → `SyncBikesCommand` loops `bikes_enabled` athletes. **Admin**: `PUT /admin/sync-activities?afterDays=`, `PUT /admin/athlete-sync?athleteId=&enabled=` (ops override).
- **1e — activities page + split sync.** Sync is two independent auto flags (`activities_enabled`, `bikes_enabled`), toggled by users on **Account → Strava** (`PUT /bike-tracker/strava/activity-sync|bike-sync`); the empty-state/banner logic keys on `(flag, count)`. First activity-enable (no history) kicks a **one-shot background `BackfillActivitiesJob`** (via `ISchedulerFactory`, `StartNow`) — rate-limit-safe. Manual bike sync (`POST .../sync-bikes` → `SyncStravaBikesCommand`) is independent of the flag. `ConnectCommand`-era `ActivateStravaSyncCommand`/`SetAthleteSyncCommand` were dropped/renamed to `SetActivitySyncCommand`(+`SetBikeSyncCommand`). WEB **Strava activities** page = first server-paged `MudTable` (`GET .../activities?page=&pageSize=` → `GetStravaActivitiesQuery`, newest first, bike resolved via `bike_link`); "Last updated" opens a history dialog (`GET .../activity-sync-history` → `GetActivitySyncHistoryQuery`, read methods on `IActivitySyncHistoryRepository`).
- **1e (iter 2) — single-activity refresh.** Each activities-page row has a **Refresh** button → `POST .../activities/{id}/refresh` → **`SyncActivityCommand { AthleteId, ActivityId }`** — an atomic single-activity sync primitive (its own token, keyed by ids, no HTTP/session coupling) that fetches `GET /activities/{id}` via `IActivityService.GetAthleteActivityAsync` (guards `athlete.id == AthleteId` → else NotFound) and `EFActivityRepository.UpsertActivityAsync` upserts **one** row (**no** delete-detection; does not touch history). The Strava client models the full **DetailedActivity** (`ActivityDetailedModel : ActivitySummaryModel` + `IActivityApi.GetActivityAsync`); persistence still uses the summary base mapping (no new columns). `SyncActivityCommand` is the reusable primitive a future **Phase 6 webhook** handler will dispatch for create/update events.
- **1b needs only `activity:read`** — it lists all the athlete's activities except "Only You"; `activity:read_all` (private/Only-You completeness) is requested up front on connect (1c-ii) and can be enabled later by existing users.
...
- **1c-ii (done): private-rides access (`activity:read_all`), asked-not-forced.** Both login and the opt-in request `Constants.Strava.AuthorizeScopes` = `read,activity:read,activity:read_all,profile:read_all` (asking `activity:read` too so declining the private part still leaves basic access). Login uses `approval_prompt=auto`; existing users change access via a standalone side-flow isolated from login: **Account → Strava** shows granted scopes + activity-access level + **Allow private rides** / **Revoke private rides access** buttons → `{authority}/account/upgrade?mode=full|basic` (`UpgradeEndpoint`, returnUrl prefix-guarded like `StartWithRedirectUriValidator`; `mode=basic` = `BasicScopes` without read_all, to downgrade) → Strava authorize (`approval_prompt=force`) → `/account/connect-upgrade` (`ConnectUpgradeEndpoint`) → `UpgradeScopeCommand` exchanges + **overwrites the athlete's token** with the (wider or narrower) `Scope`, then redirects back with `?strava_upgrade=ok|error` (the web crafts the message from its `intent` param + refreshed access level). No IdentityServer session/re-login. Authorize URL built by shared `StravaAuthorizeUrl.Build` (also used by `LoginEndpoint`). **`ConnectCommand.VerifyRequiredScope` relaxed**: activity requirement satisfied by `activity:read` OR `activity:read_all`. Sync-status returns the granted `Scopes` for display; the **Strava bikes** notice links to Account → Strava.
- Out of Phase 1c: bike mileage display (1d); webhooks (Phase 6).
...
- **Bike mileage (1d):** totals = `InitialDistanceKm/Hours/ElevationM` + Σ of the bike's attributed activities, matched `bt.bike_link.ExternalId` ↔ `strava.activity.gear_id`. Computed **on-the-fly** (D-14) by the bike queries: `EFActivityRepository.GetGearTotalsAsync` (group-by gear) → pure `BikeTotalsCalculator.Compute/Apply` → `[NotMapped]` `BikeEntity.Total*`/`AttributedActivityCount` → `BikeViewModel`. Shown in the garage (distance) + a detail "Mileage" panel. All rides count (OQ-17 filtering deferred); lifecycle-independent.
- **Components + warehouses (Phase 2a):** `bt.component` (mirrors `bt.bike` — Name/Brand/Model/`ComponentCategory`/Weight/Notes/Purchase/Initial seed + `ComponentLifecycle` Active/Archived/Sold) and `bt.warehouse` (just a name; where a non-installed component sits). `component.warehouse_id` → `bt.warehouse` is nullable, **`ON DELETE SET NULL`** (deleting a warehouse keeps its components, just clears their location). `ComponentCategory` = the CONCEPT §14 seed set as a code-side enum (D-13, **no** lookup table); `ComponentCategoryMetadata` maps each category → a `ComponentCategoryGroup` for the grouped picker (no installable flag — a per-component "cost-only" flag is a deferred idea). Full CRUD + lifecycle vertical cloned from Bike: `Save/ChangeLifecycle/Delete ComponentCommand` (+ validators), `GetComponents/GetComponentQuery` (resolves `WarehouseName`), `Save/Delete WarehouseCommand` + `GetWarehousesQuery`; `ComponentsController` (`/components`), `WarehousesController` (`/warehouses`); explicit `Component/Warehouse ViewModelMappings`; WEB `Pages/Components.razor` (+ detail), `Pages/Warehouses.razor`, `Shared/AddEditComponentDialog` (grouped category + warehouse picker), `SellComponentDialog`, `AddEditWarehouseDialog`, nav links. **No installations and no computed mileage yet** (Phase 2b / 3) — component detail shows Initial metrics only.
- **Installations (Phase 2b-i):** `bt.installation` links a component to a **bike** over a window with a position. Referenceless FKs (user Cascade, component/bike Restrict — bike installs are cleaned in code); `Type`/`Position` enums are `HasConversion<string>`. Two types (D-2b1-2): **Tracked** (`DateFrom` set, `DateTo == null` ⇒ currently installed) and **Manual** (dateless historical, static `ManualDistanceKm/Hours/ElevationM` — stored only, never computed). Invariant (D-2b1-3, D-7 subset): a component has **at most one active Tracked installation** — `InstallComponentCommand`/`MoveInstallationCommand` conflict (409) otherwise. Actions (D-2b1-4): Install / Move (atomic close-old + open-new) / Remove (set `DateTo`) / Delete (hard-delete a row) / **Update** (D-2b1-7 — edit any field except `Type`; re-checks the one-active invariant when reopening a row). Installing clears `component.warehouse_id` (D-2b1-5); component queries resolve the current install into `[NotMapped]` `InstalledOnBikeId/Name/Position`, so location reads **on-bike ▸ warehouse ▸ unassigned**. Delete guards (D-2b1-6, D-18 subset): deleting a component with any install history → Conflict (archive instead); deleting a bike calls `DeleteByBikeAsync` first. `EFInstallationRepository` (`GetByBike/Component`, `GetActiveTrackedByComponent(s)`, `AnyByComponent`, `DeleteByBikeAsync` via `ExecuteDelete`); `GetBike/GetComponentInstallationsQuery`; `InstallationsController` (`/installations` — `POST`, `PUT {id}` (update), `PUT {id}/move`, `PUT {id}/remove`, `DELETE {id}`, `GET ?bikeId=`/`?componentId=`); WEB `Shared/InstallComponentDialog` (Tracked/Manual toggle), `MoveInstallationDialog`, `RemoveInstallationDialog`, `EditInstallationDialog`; both `BikeDetails` and `ComponentDetails` render installations as **one table** (current row highlighted + "Current" chip, historical below) with per-row Edit/Move/Remove/Delete; install dates carry a time-of-day (D-2b1-8); the components list has install-state + bike filters and a row-level Install action (D-2b1-10).
- **Out of Phase 2b-i:** component→component (meta-components), one component on several bikes at once, full D-7 linkage + no-cycle, D-18 cascade/detach on parent archive/sell (Phase 2b-ii); the install-chain mileage attribution (Phase 3); alerts (5); cost analysis (4).
...
`Athletes` (koms, koms-changes, summaries, clubs, change-email), `Ranking`, `Stats` (koms-changes), `KomTakeovers` (pairs, efforts), `Bikes` (garage CRUD), `Components` (`/components` CRUD), `Warehouses` (`/warehouses` CRUD), `StravaBikes` (`/bike-tracker/strava/*` — sync, bikes list, sync-status, link), `Admin` (job triggers, takeover backfill), `Playground` (dev). All `[BearerAuthorize]`; Admin endpoints require the `admin` role.
